...
#     "要求在确保准确性的同时，尽量从文本中捕获尽可能多的信息，构建出的知识图谱要结构清晰，为你提供的文件是Markdown格式的文件，你需要根据文件的格式特点来提取文件的内容。\n"
#     "同时针对文本的内容，还需要做到：\n"
#     "- 对于文本中表格内容的抽取，请特别注意解析文件中的标题和单独换行的内容，将每个标题格视为一个独立的信息点。"
#     "如果文件中存在<!-- -->符号，符号中间的内容是图片解析的内容，请将这些图片信息作为实体的属性或与其它实体间的具体关系进行记录，并经过您的分析后对内容进行总结"
...
system_prompt = (
    ...
    "如果表格展示了具体的数值或成果，请将这些信息作为实体的属性或与其它实体间的具体关系进行记录，"
    "- 对于文本中多次提及的同一实体，使用该实体最常见且完整的名称作为实体名称。如果实体在文本中以不同名称出现，请通过算法识别并统一这些变体到同一实体名称下，以确保知识图谱的一致性和完整性。"
)
